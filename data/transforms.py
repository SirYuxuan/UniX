...
class BaseImageTransform:
    """Base image transform class that provides common image preprocessing pipeline."""
    ...
    def __call__(self, img, img_num: int = 1):
        """Apply image transformation: resize -> tensor -> normalize."""
...
class SigLIPImageTransform(BaseImageTransform):
    """Image transform class for SigLIP vision encoder with default normalization parameters."""
...
class VAEImageTransform(BaseImageTransform):
    """Image transform class for VAE model with its normalization parameters."""
...
def shuffle_patch(image, num_splits, gap_size=2):
    """Split image into patches (allowing non-divisible sizes), shuffle and stitch with gaps"""
    ...
    current_y = 0  # Starting Y coordinate of current row
    patch_idx = 0  # Current patch index being processed
    for i in range(h_splits):
        current_x = 0  # Starting X coordinate of current column
        patch_h = patch_heights[i]  # Height of current row's patch
        for j in range(w_splits):
            # Get shuffled patch
            patch = patches[patch_idx]
            patch_w = patch_widths[j]  # Width of current column's patch
            # Paste patch (top-left corner at (current_x, current_y))
            new_image.paste(patch, (current_x, current_y))
            # Update X coordinate (next patch start = current width + gap)
            current_x += patch_w + gap_size
            patch_idx += 1
        # Update Y coordinate (next row start = current height + gap)
...
def inpainting(image, num_splits, blank_ratio=0.3, blank_color=(255, 255, 255)):
    """
    Split image into patches and randomly blank some for inpainting task

    Args:
        image: PIL.Image input image (RGB mode)
        h_splits: int number of horizontal splits
        w_splits: int number of vertical splits
        blank_ratio: float ratio of blank patches (0~1)
        blank_color: tuple blank area color (RGB, e.g., white (255,255,255))

    Returns:
        PIL.Image processed and stitched image
    """
    h_splits, w_splits = num_splits
    img_w, img_h = image.size
